"""Helper methods for call summary generation - to be integrated into call_summary_generator.py"""

# These methods should be added to the CallSummaryGenerator class
...
async def generate_summary(
    ...
    try:
        import uuid
        from .call_summary_generator import CallSummary
        ...
        summary = CallSummary(
            summary_id=summary_id, room_id=room_id, call_duration_seconds=call_duration,
            total_messages=total_messages, customer_messages=len(customer_messages),
            agent_messages=len(agent_messages), customer_name=customer_name,
            customer_contact=customer_contact, call_outcome=call_outcome,
            order_placed=order_placed, order_value=order_value,
            conversation_summary=conversation_summary, key_topics=key_topics,
            books_discussed=books_discussed, genres_interested=genres_interested,
            authors_mentioned=authors_mentioned, objections_raised=objections_raised,
            concerns_addressed=concerns_addressed, unresolved_concerns=unresolved_concerns,
            overall_sentiment=overall_sentiment, sentiment_journey=sentiment_journey,
            engagement_level=engagement_level, customer_satisfaction=customer_satisfaction,
            agent_response_quality=agent_response_quality, recommendations_made=recommendations_made,
            objection_handling_score=objection_handling_score, closing_effectiveness=closing_effectiveness,
            strengths=strengths, improvement_areas=improvement_areas,
            follow_up_actions=follow_up_actions, coaching_points=coaching_points,
            generated_at=datetime.utcnow(), call_timestamp=call_timestamp, manual_notes=manual_notes
        )
